...
--
--
function primitive(nom,keys,args)
...
--
-- operator("intersection",{'convexity'},{convexity=3,child1, child2, child3, ... }
-- if there is extraargs, then merge them to the args
--
function operator(nom,keys,args,extraargs)
    local self=args -- should check that parameters are in keys, but keep elements
    if extraargs then self=tablemerge(self,extraargs) end
    self.operator=nom
    self.keys=keys
...
function bbox(a)
    ...
    return {x1=x1,x2=x2,y1=y1,y2=y2,z1=z1,z2=z2,cx=(x1+x2)/2,cy=(y1+y2)/2,cz=(z1+z2)/2}
end
...
function surface(a)
    p = poly(a)
    s = 0.0 --surface
    for i=1,#p.faces,3 do
        --3 points of a triangle
        p1 = p.faces[i]
        p2 = p.faces[i+1]
        p3 = p.faces[i+2]
        --the 2 vectors AB (p1p2) and AC (p1p3)
        vects = {{p.points[p1*3-2]-p.points[p2*3-2], p.points[p1*3-1]-p.points[p2*3-1], p.points[p1*3]-p.points[p2*3]}
                 ,{p.points[p1*3-2]-p.points[p3*3-2], p.points[p1*3-1]-p.points[p3*3-1], p.points[p1*3]-p.points[p3*3]}}
        --area of a triangle with 3d coordinates
        s = s + 1/2.0 * math.sqrt((vects[1][2]*vects[2][3] - vects[1][3]*vects[2][2])^2
                                + (vects[1][3]*vects[2][1] - vects[1][1]*vects[2][3])^2
                                + (vects[1][1]*vects[2][2] - vects[1][2]*vects[2][1])^2)
    end
    return s
end

--function export(a,name,binary)
function export(a)
    if #a > 3 then print "export{name='noname.stl',binary=0,nef) is the correct usage. Do not give more than 1 geometry to export." end
    if getmetatable(a[1])~="geometry" then error("export expects geometry") end
    local display=renderize(a[1],true)
    if #display ~= 1 then error("cant get rendering for export") end
    display[1].nef:export{name=a.name or "noname.stl",binary=a.binary or 1}
end
...
function resize(a)
    local m
    local size = bbox(a[1])
    local sizeX = size.x2-size.x1
    local sizeY = size.y2-size.y1
    local sizeZ = size.z2-size.z1
    local cx = (size.x1+size.x2)/2.0
    local cy = (size.y1+size.y2)/2.0
    local cz = (size.z1-size.z2)/2.0
    --scaleX, scaleY, scaleZ
    --translateX, translateY, translateZ
    --print("ATTENTION : ",a.x,a.x1,a.x2, a.y,a.y1,a.y2, a.z,a.z1,a.z2)
    if a.x and a.x~=0 then
        scaleX = a.x/sizeX
        translateX = 0 
    elseif a.x1 and a.x2 and a.x1~=a.x2 then
        scaleX = (a.x2-a.x1)/sizeX
        translateX = (a.x2+a.x1)/2.0 - cx
    else 
        scaleX = 1
        translateX = 0
    end

    if a.y and a.y~=0 then
        scaleY = a.y/sizeY
        translateY = 0 
    elseif a.y1 and a.y2 and a.y1~=a.y2 then
        scaleY = (a.y2-a.y1)/sizeY
        translateY = (a.y2+a.y1)/2.0 - cy
    else 
        scaleY = 1
        translateY = 0
    end
    
    if a.z and a.z~=0 then
        scaleZ = a.z/sizeZ
        translateZ = 0 
    elseif a.z1 and a.z2 and a.z1~=a.z2 then
        scaleZ = (a.z2-a.z1)/sizeZ
        translateZ = (a.z2+a.z1)/2.0 - cz
    else 
        scaleZ = 1
        translateZ = 0
    end

    --local m = {{scaleX,0,0,translateX},{0,scaleY,0,translateY},{0,0,scaleZ,translateZ},{0,0,0,1}}
    --m = matrix.scaleAround({scaleX,scaleY,scaleZ},{translateX,translateY,translateZ})
    m = matrix.scaleAround({scaleX,scaleY,scaleZ},{translateX,translateY,translateZ})
    local args = {m=m}
...
function transform(a)
    local m = a.m
    --is there 4 vector in m
    for i=#m+1,4 do m[i] = {0,0,0,0} m[i][i] = 1 end
    --is every vector of length 4
    for i=1,4 do 
        if #m[i] ~=4 then
            for j=#m[i]+1,3 do m[i][j] = 0 end
            m[i][4] = 1
        end
    end
    --no 0s on the diagonal of m, or else scale to 0
    for i=1,4 do
        --ULTRA FIX, NO GOOD, DEVRAIT ETRE UNE ERREUR PEUT ETRE?
        if m[i][i] == 0 then m[i][i] = 1 end
    end
    local args = {m=m}
    return operator("transform",{'m'},tablemerge(args,a))
end
...
function cylinder(a) return primitive("cylinder",{'h','r1','r2','n','triangles'},{h=a.h or 1,r1=a.r1 or (a.d1 and a.d1/2.0) or a.r or (a.d and a.d/2.0) or 1, r2=a.r2 or (a.d2 and a.d2/2.0) or a.r or (a.d and a.d/2.0) or 1,center=a.center or false, n=a.n or a.nb or 16, triangles=a.triangles or 0}) end -- h,r,fa fs fn
...
function cone(a) return primitive("cylinder",{'h','r1','r2','n','triangles'},{h=a.h or 1, r1=0, r2=a.r or (a.d and a.d/2.0) or a.r2 or 1, n=a.n or a.nb or 16, triangles=a.triangles or 0}) end
function import(a)
...
function rendering.cylinder(params)
    print("rendering a cylinder h="..params.h..",r1="..params.r1..",r2="..params.r2..",n="..params.n..",triangles="..params.triangles)
    return nef.cylinder{h=params.h,r1=params.r1,r2=params.r2,n=params.n,triangles=params.triangles}
end
